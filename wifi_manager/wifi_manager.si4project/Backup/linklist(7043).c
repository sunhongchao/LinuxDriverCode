...
LinkList *L; struct Node **L;这个L和上面的L不一样，
假设这个L是p,那么初始化函数如下：
struct Node *L
i=InitList(&L);
Status InitList(struct Node **p) //此时的存储格式p指向L,p->L,*P=L
{
	*p=(LinkList)malloc(sizeof(Node));//产生头结点，并使L指向此头结点
	if(!(*p))//存储分配失败
		return ERROR;
	(*p)->next=NULL;
	return OK;
...
LinkList L;
i=InitList(L);
//此时传递的不是L本身的地址，而是L的内容，传递的为空，在mylinklistdevc++.c里面验证
...
*/
Status ListInsert(LinkList *L,int i,ElemType e)//此处的L依然不是链表L，是指向L的指针
{
	int j;
	LinkList p,s;
	p=*L;//此时p相当于
