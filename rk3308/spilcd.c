...
{	
	...
	//spilcd_WriteData((1<<5)|(0<<6)|(1<<7)|(1<<3)); yuanlaide

	//spilcd_WriteData((0<<7)|(0<<6)|(1<<5)|(1<<3)); 
	spilcd_WriteData((0<<7)|(1<<6)|(1<<5)|(1<<3)); //uiot ok
	//spilcd_WriteData((1<<7)|(0<<6)|(1<<5)|(1<<3)); 
	//spilcd_WriteData((1<<7)|(1<<6)|(1<<5)|(1<<3)); 
...
static struct spilcd_data *spilcd_probe_dt(struct device *dev)
{
	...
	pdata = devm_kzalloc(dev,sizeof(struct spilcd_data), GFP_KERNEL);
	if (!pdata)
...
void set_logodata(struct fb_info *fbi)
{
	u8 *buf= (u8*)(fbi->screen_base);//内存基地址
	u32  len=fbi->fix.smem_len;
	memcpy(buf,gImage_logo,len);
}
...
void show_fb(struct fb_info *fbi, struct spi_device *spi)
{
	...
	if((colorbuf == NULL) || (fbi->screen_base == NULL)){
		printk("error:lcd = NULL\n");
		msleep(100);
	}
...
static int  spi_lcd_probe(struct spi_device *spi)
{
	...
	spilcd=devm_kzalloc(dev,sizeof(struct spilcd), GFP_KERNEL);
	if (!spilcd){
		return -ENOMEM;
	}
	spilcd->spidev=spi;
...
static long spilcd_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	printk("lcd ioctl function\n");
	switch(cmd){
		case SPILCD_ON:
			spilcd_WriteCmd(0x29); 
			schedule_delayed_work(&spilcd->delayed_worker,HZ/30);
...
static const struct file_operations spilcd_fops = {
	.owner =	THIS_MODULE,
	.open =		spilcd_open,
	.unlocked_ioctl = spilcd_ioctl,
	.release = spilcd_release,
};
static int __init spi_lcd_init(void)
{
	major = register_chrdev(0, "spilcd", &spilcd_fops);
	class = class_create(THIS_MODULE, "spilcd");
	device_create(class, NULL, MKDEV(major, 0), NULL, "spilcd");
...
static  void __exit spi_lcd_exit(void)
{
	...
	class_destroy(class);
	unregister_chrdev(major, "spilcd");
}
