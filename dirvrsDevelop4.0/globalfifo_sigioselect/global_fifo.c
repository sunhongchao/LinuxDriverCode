...
#define GLOBALFIFO_SIZE 10 /*全局内存最大4KB*/
...
static ssize_t globalmem_read(struct file *filp, char __user *buf,
				size_t size,loff_t *ppos)
 {
   ...
   else
   {
		...
		printk(KERN_DEBUG"read %d bytes(s),current_len:%d\n",count,dev->current_len);
		wake_up_interruptible(&dev->w_wait);
		if(dev->async_queue)
		{
			kill_fasync(&dev->async_queue,SIGIO,POLL_OUT);//send sigio when have space
...
static ssize_t globalmem_write(struct file *filp, const char __user 								*buf,size_t size, loff_t *ppos)
{
   ...
   {
   		dev->current_len+=count;
		printk(KERN_DEBUG"written %d bytes(s),current_len:%d\n",count,dev->current_len);
		wake_up_interruptible(&dev->r_wait);
		if(dev->async_queue)
		{
			kill_fasync(&dev->async_queue,SIGIO,POLL_IN);//send sigio when have data
			printk(KERN_DEBUG"%s kill SIGIO\n",__func__);
		}
		ret=count;
   }
